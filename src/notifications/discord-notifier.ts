...
export class DiscordNotifier implements INotifier {
  ...
  async sendCycleSummary(result: CycleResult): Promise<void> {
    ...
    await this.sendMessage(
      [
        ...
        `mode=\`${result.mode}\``,
        `scanned=${result.scanned} plans=${result.plans} approved=${result.approved} executed=${result.executed} failed=${result.failed}`
      ].join("\n")
    );
  }
  ...
  async sendAlert(level: AlertLevel, title: string, body: string, metadata: Record<string, unknown> = {}): Promise<void> {
    ...
    await this.sendMessage(
      [`**[${level.toUpperCase()}] ${title}**`, body, this.renderMetadata(metadata)].filter(Boolean).join("\n\n")
    );
  }
  ...
  private async sendMessage(content: string): Promise<void> {
    ...
    if (!response.ok) {
      ...
      const error = new Error(`Discord 通知发送失败: ${response.status} ${errorBody}`);
      this.logger.warn("Discord 通知发送失败", {
